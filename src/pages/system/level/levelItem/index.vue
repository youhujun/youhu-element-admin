 ...
 * @LastEditTime: 2024-05-26 23:19:18
-->
...
<template>
  <div>
    <Card>
      ...
      <template #cardButton>
        <el-button style="float: right; " type="primary" @click="handleLevelItem()">添加级别条件类型</el-button>
      </template>
          ...
          <ShowTable>
            ...
            <template #batch>
              ...
              <el-row type="flex" justify="space-around">
                <el-col :span="6" class="btn-action"><el-button
                  type="primary"
                  @click="handleReverse()"
                >反选</el-button></el-col>
                <el-col :span="6" class="btn-action"><el-button
            ...
            <template #table>
              ...
              <LevelItemTable
                :loading="loading"
                :level-item-list.sync="levelItemList"
                :dialog-visible.sync="dialogVisible"
                :dialog-title.sync="dialogTitle"
                :dialog-type.sync="dialogType"
                :before-level-item-form.sync="beforeLevelItemForm"
                :reverse="reverse"
    ...
    >
      ...
      <template #updateForm>
        <UpdateLevelItemForm
          v-if="dialogVisible==true"
          :before-level-item-form="beforeLevelItemForm"
          @hanldeUpdateData="toUpdateData"
          @handleCloseDialog="closeDialog"
        />
      </template>
...
import AddLevelItemForm from './component/addLevelItemForm'
import UpdateLevelItemForm from './component/updateLevelItemForm'
import Search from './component/search.vue'

import LevelItemTable from './component/levelItemTable'

import { getLevelItem } from '@/api/system/level/levelItem'
  ...
  methods:
    {
      ...
      handleLevelItem()
      {
        ...
        this.dialogTitle = '添加级别条件类型'
        this.dialogType = 1
      },
      // 获取
      async getLevelItem(where)
      {
        const levelItemData = await getLevelItem(where).then(res =>
        {
        ...
        this.currentPage = levelItemData.meta.current_page
        this.pageSize = levelItemData.meta.per_page
        this.total = levelItemData.meta.total
        this.levelItemList = levelItemData.data
      },
      handleReverse()
      {
