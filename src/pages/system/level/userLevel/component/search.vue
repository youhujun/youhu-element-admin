 ...
 * @LastEditTime: 2023-03-29 12:40:15
-->
<template>
  <div>
    <el-row :gutter="30" type="flex" class="el-row">
      <el-col :span="12">
        <el-row type="flex">
          ...
          <el-input v-model="where.find" clearable :placeholder="findSelect[where.findSelectIndex].label" class="input-with-select">
            <el-select slot="prepend" v-model="where.findSelectIndex" placeholder="请选择">
              <el-option label="用户级别名称" :value="0" />
              <el-option label="用户级别编码" :value="1" />
            </el-select>
...
export default
{
  ...
  data()
  {
    return {
      ...
      findSelect: [
        { label: '用户级别名称', value: 0 },
        { label: '用户级别编号', value: 1 }
      ],
      sortOptions: [{
        value: 1,
        label: '按时间升序'
