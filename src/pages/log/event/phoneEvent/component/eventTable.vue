 ...
 * @LastEditTime: 2024-06-02 22:38:07
-->
<template>
  <div>
    <el-table
...
import { deleteUserEventLog, multipleDeleteUserEventLog } from '@/api/log/log'
  ...
  watch:
    {
      ...
      multipleDelete(value)
      {
        ...
        this.$confirm('确认要批量删除日志吗？', '确认信息', {
          ...
          .then(() =>
          {
            multipleDeleteUserEventLog(selectId).then((res) =>
            {
              if (res && res.code === 0)
              {
  ...
  methods:
  {
    ...
    deleteEventLog(id, index)
    {
      ...
      this.$confirm('确认要删除该日志吗？', '确认信息', {
        ...
        .then(() =>
        {
          deleteUserEventLog(id).then((res) =>
          {
            if (res && res.code === 0)
            {
              that.$emit('hanldeUpdateData', true)
