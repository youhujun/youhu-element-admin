 ...
 * @LastEditTime: 2024-06-02 21:43:30
-->
<template>
  <div>
    <el-table
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <el-tag v-show="scope.row.status==10" type="success">管理员登录</el-tag>
          <el-tag v-show="scope.row.status==20" type="info">管理员退出</el-tag>
        </template>
      </el-table-column>
      <el-table-column
        align="center"
...
import { deleteAdminLoginLog, multipleDeleteAdminLoginLog } from '@/api/log/log'
export default
{
  // 组件名称
  name: 'LoginTable',
  ...
  watch:
    {
      ...
      multipleDelete(value)
      {
        ...
        this.$confirm('确认要批量删除日志吗？', '确认信息', {
          ...
          .then(() =>
          {
            multipleDeleteAdminLoginLog(selectId).then((res) =>
            {
              if (res && res.code === 0)
              {
  ...
  methods:
  {
    ...
    deleteLoginLog(id, index)
    {
      ...
      this.$confirm('确认要删除该日志吗？', '确认信息', {
        ...
        .then(() =>
        {
          deleteAdminLoginLog(id).then((res) =>
          {
            if (res && res.code === 0)
            {
              that.$emit('hanldeUpdateData', true)
