 ...
 * @LastEditTime: 2024-06-25 23:28:27
-->
<template>
  <div style="margin-bottom:1rem;">
    ...
    <el-row v-if="showResult && imageList.length === 1 && !hasPicture" style="margin-top:0.5rem;" type="flex" justify="center">
      <el-col v-for="(item,key) in imageList" :key="key" :span="12">
        <el-image style="width: 100px; height: 100px;" :src="item.picture" fit="fill" @click="handleClickPicture(item)" />
      </el-col>
    </el-row>
    <el-row v-if="hasPicture" style="margin-top:0.5rem;" type="flex" justify="center">
      <el-col :span="12">
        <el-image style="width: 100px; height: 100px;" :src="beforePictureString" fit="fill" @click="handleClickPicture(beforePictureString)" />
      </el-col>
    </el-row>
  </div>
</template>
...
export default
{
  ...
  mounted()
  {
    this.singlePicture = this.baseUrl + 'uploadSinglePicture'
  },
  // 更新之前
  beforeUpdate()
  {
  ...
  methods:
  {
    ...
    submitUpload()
    {
      ...
      uploadSinglePicture(this.formData).then(res =>
      {
        if (res && res.code === 0)
        {
          ...
          this.$emit('update:pictureId', String(res.data.id))
    ...
    handleToConfirm()
    {
      ...
      this.$emit('update:pictureId', pictureId.join())
